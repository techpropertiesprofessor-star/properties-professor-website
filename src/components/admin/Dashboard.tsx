...
export function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const analyticsRes = await fetch('http://localhost:5001/api/analytics/dashboard', {
        headers: {
      ...
      // Fetch recent leads
      const leadsRes = await fetch('http://localhost:5001/api/leads?limit=5&sort=-createdAt', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
